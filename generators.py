...
def dialog(name=None):
    ...
    update = yield Message(r'Привет, я Александр Александрович В и я очень люблю платки и шарфики! Меня создали для того, что бы помочь тебе выглядеть стильно и изящно с помощью твоего красивого платочка/шарфика! Хочешь увидеть варианты? Пиши /next, и я пришлю тебе картинку и инструкцию, как это сделать.')
    image = MyImage()
    # image = MyLinkImage()
    answer = update.message
    while True:
        if answer.text.startswith('/help'):
            ...
            update = yield Message('Меня создали для того, что бы помочь тебе выглядеть стильно и изящно с помощью твоего красивого платочка/шарфика! Хочешь увидеть варианты? Пиши /next, и я пришлю тебе картинку и инструкцию, как это сделать.')
            answer = update.message
            continue
...
def MyLinkImage():
    ...
    for i in l:
        ...
        with open(str('Images/' + str(i)), 'r') as r:
            text = r.read()
        yield PhotoMessage(text, '/next для следующего способа', photo=link)
